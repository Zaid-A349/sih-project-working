...
const LoginPage = ({ handleLogin }) => {
  ...
  const getTabClass = (tabName) => `cursor-pointer py-3 px-6 text-center text-lg font-semibold rounded-t-lg transition-all duration-300 ${activeTab === tabName ? 'bg-slate-100 text-indigo-600' : 'text-gray-300 hover:bg-slate-500/10'}`;
  ...
  return (
    <div className="min-h-screen bg-cover bg-center font-sans" style={{ backgroundImage: `url(${backgroundImage})` }}>
      <div className="min-h-screen bg-black/60 backdrop-blur-sm flex flex-col items-center justify-center p-4">
        ...
        {showInstituteLogin ? (
          <button onClick={() => handleTabClick('student')} className="absolute top-6 right-6 flex items-center gap-2 px-4 py-2 font-semibold text-white transition-colors duration-300 bg-white/20 hover:bg-white/30 rounded-lg backdrop-blur-sm">
            <Users size={20} />
            Student / Faculty Login
          </button>
        ) : (
          <button onClick={handleInstituteClick} className="absolute top-6 right-6 flex items-center gap-2 px-4 py-2 font-semibold text-white transition-colors duration-300 bg-white/20 hover:bg-white/30 rounded-lg backdrop-blur-sm">
            <Building2 size={20} />
            Institute Login
          </button>
        )}
        ...
        <div className="w-full max-w-md">
          <div className="bg-white/10 backdrop-blur-xl rounded-xl shadow-2xl overflow-hidden">
            {!showInstituteLogin && (
